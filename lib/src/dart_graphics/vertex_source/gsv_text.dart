...
/// Simple vector text renderer using built-in font data.
/// Port of DartGraphics's gsv_text class.
class GsvText implements IVertexSource {
  double _startX = 0.0;
  double _y = 0.0;
  double _height = 10.0;
  double _width = 0.0;
  double _space = 0.0;
  double _lineSpace = 0.0;
  String _text = '';
  bool _flip = false;
  
  // Iteration state
  _GsvTextStatus _status = _GsvTextStatus.initial;
  int _charIndex = 0;
  int _glyphBegin = 0;
  int _glyphEnd = 0;
  ...
  /// Creates a new GsvText instance
  GsvText();
  
  /// Sets the text size.
  /// [height] - the height of the text
  /// [width] - optional width (if 0, uses height for aspect ratio)
  ...
  void lineSpace(double lineSpace) {
    _lineSpace = lineSpace;
  }
  
  /// Sets the starting point for text rendering
  void startPoint(double x, double y) {
    _startX = x;
    _y = y;
  }
  
  /// Sets whether to flip the text vertically
  void flip(bool flip) {
    _flip = flip;
  }
  ...
  void rewind([int pathId = 0]) {
    _status = _GsvTextStatus.initial;
    _charIndex = 0;
    _currentX = _startX;
    _currentY = _y;
  }
  ...
  FlagsAndCommand vertex(RefParam<double> x, RefParam<double> y) {
    final font = _gsvDefaultFont;
    
    // Calculate indices and glyphs offsets
    final indices = _value16(font, 0);
    final glyphs = indices + 257 * 2;
    
    // Calculate scaling
    final baseHeight = _value16(font, 4).toDouble();
    var hi = _height / baseHeight;
    final wi = _width == 0.0 ? hi : _width / baseHeight;
    if (_flip) {
      hi = -hi;
    }
    ...
    while (true) {
      switch (_status) {
        case _GsvTextStatus.initial:
          _currentX = _startX;
          _currentY = _y;
          _charIndex = 0;
          _status = _GsvTextStatus.nextChar;
          continue;
          
        case _GsvTextStatus.nextChar:
          ...
          final chr = _text.codeUnitAt(_charIndex);
          _charIndex++;
          
          if (chr == 0x0A || chr == 0x0D) { // newline
            _currentX = _startX;
            _currentY -= _flip 
                ? -(_height + _lineSpace) 
                : (_height + _lineSpace);
          ...
          final idx = (chr & 0xFF) * 2;
          _glyphBegin = glyphs + _value16(font, indices + idx);
          _glyphEnd = glyphs + _value16(font, indices + idx + 2);
          _status = _GsvTextStatus.startGlyph;
          continue;
          
        case _GsvTextStatus.startGlyph:
          x.value = _currentX;
          y.value = _currentY;
          _status = _GsvTextStatus.glyph;
          return FlagsAndCommand.commandMoveTo;
          
        case _GsvTextStatus.glyph:
          if (_glyphBegin >= _glyphEnd) {
            _currentX += _space;
            _status = _GsvTextStatus.nextChar;
            continue;
          }
          
          final dx = font[_glyphBegin].toSigned(8);
          var yc = font[_glyphBegin + 1].toSigned(8);
          final yf = font[_glyphBegin + 1] & 0x80;
          
          // Sign extend yc (remove the flag bit)
          yc = ((yc << 1) & 0xFF).toSigned(8) >> 1;
          final dy = yc;
  ...
  Iterable<VertexData> vertices() sync* {
    ...
    while (true) {
      final cmd = vertex(x, y);
      if (cmd.isStop) break;
      yield VertexData(cmd, x.value, y.value);
    }
  }
  
  /// Helper to read a 16-bit little-endian value from font data
  int _value16(Uint8List data, int offset) {
    return data[offset] | (data[offset + 1] << 8);
  }
}

/// Default font data - embedded vector font
final Uint8List _gsvDefaultFont = Uint8List.fromList([
