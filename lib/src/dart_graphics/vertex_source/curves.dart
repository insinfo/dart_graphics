...
class Curve3Div {
  ...
  void recursive_bezier(double x1, double y1, double x2, double y2, double x3,
    ...
    if (d > Curves.curve_collinearity_epsilon) {
      ...
      da = dx * dx + dy * dy;
      if (da == 0) {
        d = DartGraphicsMath.calc_sq_distance(x1, y1, x2, y2);
      } else {
        ...
        if (d <= 0)
          d = DartGraphicsMath.calc_sq_distance(x2, y2, x1, y1);
        else if (d >= 1)
          d = DartGraphicsMath.calc_sq_distance(x2, y2, x3, y3);
        else
          d = DartGraphicsMath.calc_sq_distance(x2, y2, x1 + d * dx, y1 + d * dy);
      }
  ...
  void recursive_bezier(double x1, double y1, double x2, double y2, double x3,
    ...
    switch (switchCase) {
      case 0:
        k = dx * dx + dy * dy;
        if (k == 0) {
          d2 = DartGraphicsMath.calc_sq_distance(x1, y1, x2, y2);
          d3 = DartGraphicsMath.calc_sq_distance(x4, y4, x3, y3);
        } else {
          k = 1 / k;
          da1 = x2 - x1;
          da2 = y2 - y1;
          ...
          if (d2 <= 0)
            d2 = DartGraphicsMath.calc_sq_distance(x2, y2, x1, y1);
          else if (d2 >= 1)
            d2 = DartGraphicsMath.calc_sq_distance(x2, y2, x4, y4);
          else
            d2 = DartGraphicsMath.calc_sq_distance(x2, y2, x1 + d2 * dx, y1 + d2 * dy);

          if (d3 <= 0)
            d3 = DartGraphicsMath.calc_sq_distance(x3, y3, x1, y1);
          else if (d3 >= 1)
            d3 = DartGraphicsMath.calc_sq_distance(x3, y3, x4, y4);
          else
            d3 = DartGraphicsMath.calc_sq_distance(x3, y3, x1 + d3 * dx, y1 + d3 * dy);
        }
        if (d2 > d3) {
