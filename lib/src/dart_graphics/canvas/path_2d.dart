/// Path2D - DartGraphics implementation of HTML5 Canvas Path2D
///
...
class DartGraphicsPath2D implements IPath2D {
  final List<_PathCommand> _commands = [];

  /// Returns true if the path contains curved segments (arc/ellipse/bezier/etc).
  bool get hasCurves {
  ...
  factory DartGraphicsPath2D.fromSvgPath(String svgPath) {
    final path = DartGraphicsPath2D();
    path.addSvgPath(svgPath);
    return path;
  }

  /// Copies another Path2D
  factory DartGraphicsPath2D.from(DartGraphicsPath2D other) {
    final path = DartGraphicsPath2D();
  ...
  void addPath(covariant IPath2D path, [DOMMatrix? transform]) {
    if (path is! DartGraphicsPath2D) return;
    if (transform == null) {
      _commands.addAll(path._commands);
    } else {
      // Convert DOMMatrix to DartGraphicsMatrix2D
      final DartGraphicsTransform = DartGraphicsMatrix2D(transform.a,
          transform.b, transform.c, transform.d, transform.e, transform.f);
      for (final cmd in path._commands) {
        _commands.add(cmd.transformed(DartGraphicsTransform));
      }
    }
  }
  ...
  void arc(
    ...
    _commands.add(
        _PathCommand.arc(x, y, radius, startAngle, endAngle, counterclockwise));
  }
  ...
  void ellipse(double x, double y, double radiusX, double radiusY,
      ...
      [bool counterclockwise = false]) {
    _commands.add(_PathCommand.ellipse(x, y, radiusX, radiusY, rotation,
  ...
  void roundRect(double x, double y, double width, double height,
    ...
    if (radii != null) {
      if (radii is num) {
        ...
        if (radii.length == 1) {
          topLeft = topRight =
              bottomRight = bottomLeft = (radii[0] as num).toDouble();
    ...
    _commands.add(_PathCommand.roundRect(
  ...
  /// Converts this path to an DartGraphics VertexStorage with curves flattened to line segments
  ///
  ...
  void _flattenCubicBezier(VertexStorage vs, double x0, double y0, double cp1x,
      double cp1y, double cp2x, double cp2y, double x, double y) {
    const int segments = 60;
    for (int i = 1; i <= segments; i++) {
      final t = i / segments;
      final t2 = t * t;
  ...
  /// Converts this path to an DartGraphics VertexStorage
  ...
  void _addArcToVertexStorage(
    ...
    final segments = math.max(8, (sweep.abs() * radius / 2).round());
  ...
  void _addArcToPathToVertexStorage(VertexStorage vs, double x0, double y0,
    ...
    // Draw arc
    _addArcToVertexStorage(
        vs, cx, cy, radius, startAngle, endAngle, sinAngle < 0, t1x, t1y, true);
  }

  void _addEllipseToVertexStorage(
      VertexStorage vs,
      double cx,
      double cy,
      double rx,
    ...
    final segments = math.max(12, (sweep.abs() * math.max(rx, ry) / 2).round());
    final step = sweep / segments;
  ...
  void _addRoundRectToVertexStorage(VertexStorage vs, double x, double y,
      double w, double h, double tl, double tr, double br, double bl) {
  ...
  void _parseSvgPath(String svgPath) {
    ...
    final commands =
        RegExp(r'([MmLlHhVvCcSsQqTtAaZz])([^MmLlHhVvCcSsQqTtAaZz]*)');
      ...
      switch (cmd.toUpperCase()) {
        case 'M':
          ...
          while (i < args.length - 1) {
            final x = args[i] +
                (isRelative && i > 0 ? currentX : (isRelative ? currentX : 0));
            final y = args[i + 1] +
                (isRelative && i > 0 ? currentY : (isRelative ? currentY : 0));
            if (i == 0) {
              moveTo(x, y);
              startX = x;
              startY = y;
        ...
        case 'A':
          ...
          while (i < args.length - 6) {
            ...
            _addSvgArc(
                currentX, currentY, rx, ry, rotation, largeArc, sweep, x, y);
            currentX = x;
            currentY = y;
            i += 7;
          }
  ...
  void _addSvgArc(double x1, double y1, double rx, double ry, double rotation,
    ...
    ellipse(
        cx, cy, rx, ry, rotation, startAngle, startAngle + deltaAngle, !sweep);
  }
...
class _PathCommand {
  ...
  factory _PathCommand.bezierCurveTo(
      double cp1x, double cp1y, double cp2x, double cp2y, double x, double y) {
    return _PathCommand._(
        type: _PathCommandType.bezierCurveTo,
        cp1x: cp1x,
        cp1y: cp1y,
        cp2x: cp2x,
        cp2y: cp2y,
  ...
  factory _PathCommand.quadraticCurveTo(
      double cpx, double cpy, double x, double y) {
    return _PathCommand._(
        type: _PathCommandType.quadraticCurveTo,
        cpx: cpx,
        cpy: cpy,
        x: x,
        y: y);
  ...
  factory _PathCommand.arc(double x, double y, double radius, double startAngle,
      double endAngle, bool counterclockwise) {
    return _PathCommand._(
        type: _PathCommandType.arc,
        x: x,
        y: y,
        radius: radius,
  ...
  factory _PathCommand.arcTo(
      double x1, double y1, double x2, double y2, double radius) {
    return _PathCommand._(
        type: _PathCommandType.arcTo,
        x1: x1,
        y1: y1,
        x2: x2,
        y2: y2,
  ...
  factory _PathCommand.rect(double x, double y, double width, double height) {
    return _PathCommand._(
        type: _PathCommandType.rect, x: x, y: y, width: width, height: height);
  }

  factory _PathCommand.roundRect(
      double x,
      double y,
      double width,
      double height,
      ...
      double bottomLeft) {
    return _PathCommand._(
        type: _PathCommandType.roundRect,
        x: x,
        y: y,
        width: width,
        height: height,
  ...
  _PathCommand transformed(DartGraphicsMatrix2D t) {
    switch (type) {
      case _PathCommandType.moveTo:
      ...
      case _PathCommandType.arc:
        ...
        return _PathCommand.arc(
            c.x, c.y, radius!, startAngle!, endAngle!, counterclockwise!);
      ...
      case _PathCommandType.ellipse:
        final c = t.transformPoint(x!, y!);
        return _PathCommand.ellipse(c.x, c.y, radiusX!, radiusY!, rotation!,
            startAngle!, endAngle!, counterclockwise!);
      ...
      case _PathCommandType.roundRect:
        final p = t.transformPoint(x!, y!);
        return _PathCommand.roundRect(p.x, p.y, width!, height!, topLeft!,
