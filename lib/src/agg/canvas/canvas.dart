/// Canvas class - HTML5-style canvas for 2D drawing using AGG
/// 
...
/// An HTML5-style Canvas element for 2D drawing operations using AGG
/// 
/// This class wraps an AGG ImageBuffer and provides the familiar Canvas API.
class AggHtmlCanvas implements IHtmlCanvas {
  int _width;
  int _height;
  ImageBuffer? _buffer;
  AggCanvasRenderingContext2D? _ctx;
  
  /// Creates a new Canvas with the specified dimensions
  AggHtmlCanvas(this._width, this._height) {
    _createBuffer();
  }
  ...
  /// The underlying AGG ImageBuffer
  ...
  @override
  AggCanvasRenderingContext2D getContext(String contextType) {
    if (contextType != '2d') {
      throw ArgumentError("Only '2d' context is supported");
    }
    _ctx ??= AggCanvasRenderingContext2D(this);
    return _ctx!;
  }
  ...
  bool saveAs(String filename, {int quality = 90}) {
    ...
    switch (extension) {
      case 'png':
        ...
        return true;
      default:
        // AGG only supports PNG natively
        PngEncoder.saveImage(_buffer!, filename);
        return true;
    }
  }
  ...
  /// 
  /// Only 'image/png' is supported by AGG
  @override
  ...
  /// Creates a Blob (not implemented for AGG, throws)
  @override
