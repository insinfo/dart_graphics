...
import 'cairo_api.dart' show Cairo;
...
class CairoSurface {
  ...
  CairoBindings libcairo;

  CairoSurface._(this._ptr, this.width, this.height, this.format,
      {CairoBindings? cairoBinding})
      : libcairo = cairoBinding ?? loadCairo();
  ...
  factory CairoSurface.image(int width, int height,
      ...
      CairoBindings? cairoBinding}) {
    final libcairo = cairoBinding ?? loadCairo();
    final ptr = libcairo.cairo_image_surface_create(format, width, height);
    return CairoSurface._(ptr, width, height, format, cairoBinding: libcairo);
  }
...
class CairoCanvas {
  ...
  /// Font cache for this canvas (lazy initialized)
  static CairoFontCache? _fontCache;
  static CairoFontCache get _fonts => _fontCache ??= CairoFontCache(Cairo());

  /// Get the Cairo bindings from the surface
  CairoBindings get libcairo => surface.libcairo;

  /// Create a new canvas with the given dimensions
  factory CairoCanvas(int width, int height,
      {CairoFormatEnum format = CairoFormatEnum.CAIRO_FORMAT_ARGB32,
      CairoBindings? cairoBinding}) {
    final surface = CairoSurface.image(width, height,
        format: format, cairoBinding: cairoBinding);
    return CairoCanvas.fromSurface(surface);
  }

  /// Create a canvas from an existing surface
  CairoCanvas.fromSurface(this.surface)
      : _ctx = surface.libcairo.cairo_create(surface._ptr);
  ...
  CairoCanvas setFontFaceFromFile(String fontPath, {int faceIndex = 0}) {
    final face = _fonts.getOrLoad(fontPath, faceIndex: faceIndex);
    if (face == null) {
