/// Golden Comparison Tests - Compare DartGraphics port output with Cairo reference
/// 
/// These tests compare the images generated by the Dart DartGraphics port with
/// reference images generated by Cairo to validate rendering quality.
///
/// PSNR thresholds:
/// - Shapes: >30 dB (pixel-perfect required - same algorithm)
/// - Text: >14 dB (different font engines - Cairo/FreeType vs DartGraphics/Typography)
...
/// Cairo uses FreeType rasterizer, DartGraphics uses its own Typography engine
...
void main() {
  ...
  group('Golden Comparison - Shapes (PIXEL-PERFECT required)', () {
    ...
    test('shape_bezier: DartGraphics vs Cairo - BEZIER CURVES', () {
      ...
      expect(result.psnr, greaterThan(kPixelPerfectPsnr), 
        reason: 'Bezier curves must be pixel-perfect. DartGraphics is drawing LINES instead of CURVES! PSNR: ${result.psnr.toStringAsFixed(2)} dB');
    });
  ...
  group('Golden Comparison - Anti-aliasing (algorithm differences expected)', () {
    test('aa_diagonals: DartGraphics vs Cairo - DIAGONAL LINES', () {
      if (!File('test/tmp/aa_diagonals.png').existsSync()) {
        fail('test/tmp/aa_diagonals.png does not exist - run DartGraphics tests first');
      }
    ...
    test('aa_small_circles: DartGraphics vs Cairo - SMALL CIRCLES', () {
      if (!File('test/tmp/aa_small_circles.png').existsSync()) {
        fail('test/tmp/aa_small_circles.png does not exist - run DartGraphics tests first');
      }
