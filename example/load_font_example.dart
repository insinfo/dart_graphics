...
void main(List<String> args) {
  if (args.isEmpty) {
    print(
        'Uso: dart run example/load_font_example.dart <caminho_para_arquivo.ttf>');
    print('\nExemplos:');
    print(
  ...
  try {
    ...
    final typeface = _readTypeface(reader, bytes);

    if (typeface == null) {
      print('Falha ao ler a fonte. Verifique se o arquivo é TTF/OTF válido.');
      return;
    }

    print('Fonte carregada com sucesso!\n');
    _printFontInfo(typeface);
    _demonstrateTextLayout(typeface);
    _demonstrateCharacterMetrics(typeface);
  } on UnimplementedError catch (e) {
    print('Erro: ${e.message ?? 'Funcionalidade não implementada.'}');
    print('Dica: arquivos .ttc (coleções) ainda não são suportados.');
  } catch (e, stackTrace) {
...
Typeface? _readTypeface(OpenFontReader reader, Uint8List bytes) {
  try {
...
void _demonstrateTextLayout(Typeface typeface) {
  ...
  for (final text in texts) {
    ...
    print(
        '    Largura: $totalWidth unidades (${widthInPixels.toStringAsFixed(2)}px @ 16pt)');
  }
  print('');
}
...
void _demonstrateCharacterMetrics(Typeface typeface) {
  ...
  for (final char in testChars) {
    ...
    if (glyphIndex > 0) {
      ...
      print(
          '  $charDisplay    $unicode  ${glyphIndex.toString().padLeft(5)}  ${advance.toString().padLeft(7)}  ${lsb.toString().padLeft(3)}');
    }
  }
  print('');
  ...
  for (final pointSize in sizes) {
    ...
    print(
        '${pointSize.toString().padLeft(12)}  ${pixels.toStringAsFixed(2).padLeft(8)}  ${scale.toStringAsFixed(6)}');
  }
  print('');
}
